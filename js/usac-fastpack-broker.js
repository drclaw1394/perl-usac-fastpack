...
class uSACFastPackBrokerBridge {
  constructor(broker){
    ...
    this.forward_message_sub= (arr, cb)=>{
      ...
      let arg={buffer:undefined, inputs:undefined};
    ...
    // messages
    let arg={buffer:undefined, outputs:[]};

    this.on_read_handler=(arr, cb)=>{

      arg.buffer.append(arr[0]);

      fastpack.decode_message(arg);

      for(let i=0; i <arg.outputs.length; i++){
        let msg=arg.outputs[i];
        if(msg.id == '0'){
...
class uSACFastPackBrokerBridgeWS extends uSACFastPackBrokerBridge {
  ...
  constructor(broker, ws){
    ...
    this.ws.on_message=
      (message)=>{
      this.on_read_handler([message], undefined); 
    };
...
class uSACFastPackBroker {
              ...
              for(let k=this.ht.table.length-1; k>=0; k--){
                ...
                console.log("ignore name and type match",e)
                  for(let l=e.value.length-2; l >=0; l-=2){
                    if((e.value[l] == sub ) && (e.value[l+1] == source_id)){
                      e.value.splice(l,2);
                      found=1;
                      continue;
                    }
    ...
    broadcast(name, value){
      this.broadcaster_sub(undefined, [name, value]);
    }

    listen(name, value){
      this.listener_sub(undefined, name, value);
    }

    ignore(name, value){
      this.ignorer_sub(undefined, name, value);
    }
